...
function Overview() {
  ...
  const { data } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data: products } = await supabase.from("products").select("id,stock").in("farm_id", farmIds.length ? farmIds : ["00000000-0000-0000-0000-000000000000"]);
      const { data: items } = await supabase.from("order_items").select("qty,unit_price_paise,order_id").in("farm_id", farmIds.length ? farmIds : ["00000000-0000-0000-0000-000000000000"]);
  ...
  return (
    <div>
      ...
      <p className="font-mono text-xs uppercase tracking-widest text-muted-foreground mb-8">Your farm at a glance</p>
