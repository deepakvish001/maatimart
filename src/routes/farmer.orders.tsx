...
function FarmerOrders() {
  ...
  const { data } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data } = await supabase
        .from("orders")
        .select("id,status,total_paise,address,created_at,order_items!inner(product_name,qty,unit,unit_price_paise,farm_id)")
  ...
  return (
    <div>
      ...
      <p className="font-mono text-xs uppercase tracking-widest text-muted-foreground mb-8">Manage shipments</p>
