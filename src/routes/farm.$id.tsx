...
function FarmPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data: products } = await supabase
        .from("products")
        .select(
          "id,name,unit,price_paise,image_url,is_organic,stock,rating_avg,rating_count,created_at",
        )
        .eq("farm_id", id)
        .eq("is_active", true);
  ...
  const sortedProducts = useMemo(() => {
    ...
    switch (sort) {
      case "price-asc":
        ...
        break;
      case "price-desc":
        list.sort((a, b) => b.price_paise - a.price_paise);
        break;
      case "rating":
  ...
  const yearsActive = Math.max(
    1,
    new Date().getFullYear() - new Date(farm.created_at).getFullYear(),
  );
  ...
  const badges = [
    ...
    farm.delivery_pincodes?.length > 0 && {
      icon: Truck,
      label: `Delivers to ${farm.delivery_pincodes.length} areas`,
    },
        ...
        <section className="border-b border-border">
          <div className="mx-auto max-w-7xl px-4 md:px-6 py-10 md:py-12 grid lg:grid-cols-2 gap-8 lg:gap-12 items-center">
            <div>
              ...
              <h1 className="font-display text-4xl md:text-5xl lg:text-6xl font-bold mt-3 mb-2">
                {farm.name}
              </h1>
              ...
              {stats.total > 0 && (
                <div className="mb-5">
                  <StarRating value={stats.avg} count={stats.total} size={18} />
                </div>
              )}
              ...
              <div className="mt-6 flex flex-wrap gap-2">
                {badges.map((b, i) => (
                  <span
                    key={i}
                    className="inline-flex items-center gap-1.5 rounded-full bg-primary/10 text-primary px-3 py-1.5 text-xs font-semibold"
                  >
            ...
            <div className="relative">
              {img && (
                <img
                  src={img}
                  alt={farm.name}
                  className="aspect-square w-full object-cover rounded-2xl shadow-lg"
                />
              )}
            ...
            <div className="mx-auto max-w-7xl px-4 md:px-6 py-10 grid md:grid-cols-3 gap-8 items-center">
              <div className="text-center md:text-left">
                ...
                <p className="text-sm text-muted-foreground mt-2">
                  {stats.total} review{stats.total === 1 ? "" : "s"} across this farm
                </p>
              </div>
          ...
          <div className="flex flex-wrap items-end justify-between gap-4 mb-6">
            <div>
              ...
              <p className="text-sm text-muted-foreground mt-1">
                {stats.productCount} active product{stats.productCount === 1 ? "" : "s"}
              </p>
            </div>
          ...
          {sortedProducts.length === 0 ? (
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-px bg-border border border-border rounded-2xl overflow-hidden">
              {sortedProducts.map((p) => (
                <ProductCard
                  key={p.id}
                  p={{ ...p, farm: { name: farm.name, region: farm.region } }}
                />
              ))}
            </div>
          )}
